...
For this the framework will provide a structure that the developer uses to mark this kind of events.
The events then must be passed as an argument into a predefined "intent" function, which only takes marked events as a valid input.
Thus the developer is forced to name every event that can possibly happen. The "intent" function itself servers as an entrypoint sto the unidirectional flow 
of the framework (and so MVI).
\\
\\
